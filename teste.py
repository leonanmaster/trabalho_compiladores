...
def gerar_gabarito():
    ...
    with open("gabarito.txt", "w", encoding="utf-8") as escrita:
        for numero, teste in enumerate(testes, start=1):
            saida = executar_make(teste)
            escrita.write(SEPARADOR_SAIDA.format(numero=numero))
            escrita.write("\n")
...
def compare():
    ...
    with open("gabarito.txt", "r", encoding="utf-8") as saidas:
    ...
    for numero, teste in enumerate(testes, start=1):
        ...
        if numero > len(saidas_esperadas):
            print(f"Teste {numero}: saida esperada nao encontrada em gabarito.txt")
            testes_falhos.append(numero)
            continue
    ...
    if len(saidas_esperadas) > len(testes):
        print(f"{len(saidas_esperadas) - len(testes)} saida(s) esperada(s) sobrando em gabarito.txt")
        testes_falhos.append("gabarito extra")

    print()
    if testes_falhos:
        print("Resumo: alguns testes falharam.")
        print("Testes com problema:", ", ".join(str(teste) for teste in testes_falhos))
    else:
        print(f"Resumo: todos os {len(testes)} testes funcionaram.")

def main():
     parser = argparse.ArgumentParser()

     parser.add_argument("-m", choices=["store","compare","gerar_gabarito"])
